...
impl OutputFormat {
    pub fn from_str(s: &str) -> Self {
        match s.to_lowercase().as_str() {
            ...
            "compact" | "prompt" => OutputFormat::Compact,
...
impl Renderer {
    ...
    fn render_markdown(package: &OutputPackage) -> Result<String> {
        ...
        output.push_str(&format!("- **Total Input Tokens**: {}\n", package.metadata.total_input_tokens));
        ...
        output.push_str(&format!("- **Used Tokens**: {}\n", package.budget.used_tokens));
        ...
        if !package.validation.errors.is_empty() {
            ...
            for error in &package.validation.errors {
                output.push_str(&format!("- **{}**: {} ({:?})\n", error.path, error.message, error.error_type));
            }
        }
        if !package.validation.repairs.is_empty() {
            output.push_str("\n### Repairs\n\n");
            for repair in &package.validation.repairs {
                output.push_str(&format!("- **{}**: {} ({:?})\n", repair.path, repair.description, repair.action));
            }
        }
        output.push('\n');
        ...
        for (i, chunk) in package.chunks.iter().enumerate() {
            ...
            output.push_str(&format!("- **Offsets**: {}-{}\n\n", chunk.source.offset_start, chunk.source.offset_end));
            output.push_str("```\n");
    ...
    fn render_compact(package: &OutputPackage) -> Result<String> {
        ...
        output.push_str("=== COMPILED PROMPT PACKAGE ===\n");
        output.push_str(&format!("Timestamp: {}\n", package.metadata.timestamp));
        output.push_str(&format!("Budget: {}/{} tokens\n", package.budget.used_tokens, package.budget.max_tokens));
        ...
        for chunk in &package.chunks {
            if let Some(file) = &chunk.source.file {
                output.push_str(&format!("{}: {} (lines {}-{}, {} tokens)\n", 
                    chunk.id, 
                    file.display(),
                    chunk.line_start,
                    chunk.line_end,
    ...
    pub fn render_chunks_only(chunks: &[Chunk], format: OutputFormat) -> Result<String> {
        match format {
...
mod tests {
    use super::*;
    use crate::types::{SourceInfo, ErrorType};
    ...
    fn test_render_compact() {
        ...
        let result = Renderer::render_package(chunks, budget, validation, metadata, OutputFormat::Compact);
        assert!(result.is_ok());
    ...
    fn test_render_json() {
        ...
        let result = Renderer::render_package(chunks, budget, validation, metadata, OutputFormat::Json);
        assert!(result.is_ok());
    ...
    fn test_output_format_from_str() {
        ...
        assert!(matches!(OutputFormat::from_str("JSON"), OutputFormat::Json));
        assert!(matches!(OutputFormat::from_str("markdown"), OutputFormat::Markdown));
        assert!(matches!(OutputFormat::from_str("md"), OutputFormat::Markdown));
        assert!(matches!(OutputFormat::from_str("compact"), OutputFormat::Compact));
        assert!(matches!(OutputFormat::from_str("prompt"), OutputFormat::Compact));
    }
}
